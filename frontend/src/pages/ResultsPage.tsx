...
const ResultsPage = () => {
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: `I scored ${currentResultToDisplay?.totalScore}/100 on my ${jobRoleDisplay} interview on InterviewPrep!`,
  ...
  const handleDownload = () => {
    if (!currentResultToDisplay) return;
    const lines = [
      `InterviewPrep Interview Results`,
      `Date: ${formatDate(currentResultToDisplay.date)}`,
      `Role: ${jobRoleDisplay} | Level: ${experienceLevelDisplay}`,
      `Overall Score: ${currentResultToDisplay.totalScore}/100`,
      ``,
      `Feedback: ${currentResultToDisplay.feedback}`,
      ``,
      `Strengths:`,
      ...currentResultToDisplay.strengths.map((s: string) => `  - ${s}`),
      ``,
      `Areas for Improvement:`,
      ...currentResultToDisplay.improvements.map((i: string) => `  - ${i}`),
      ``,
      `--- Answer Details ---`,
      ...currentResultToDisplay.answers.map((a: InterviewAnswer, idx: number) => [
    ...
    anchor.href = url;
    anchor.download = `interviewprep-results-${jobRoleDisplay.replace(/ /g, '-')}-${new Date().toISOString().slice(0, 10)}.txt`;
    anchor.click();
